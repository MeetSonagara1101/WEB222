...
function toCamelCase(name, uppercase = false) {
  let result = name.replace(/(_[a-z])/g, (match) => {
...
function createLinkTag(rel, href) {
  ...
  let rel1 = /\s/.test(rel) ? `rel="${rel}"` : `rel=${rel}`;
  let href1 = /\s/.test(href) ? `href="${href}"` : `href=${href}`;

  return `<link ${rel1} ${href1}>`;
}
...
function parseDateString(value) {
  ...
  let formatMatched = dateFormats.find((dateFormat) => dateFormat.regular_exp.test(value));
  ...
  let match = value.match(formatMatched.regular_exp);
  let parsedYear = parseInt(match[formatMatched.yearIndex], 10);
  let parsedMonth = parseInt(match[formatMatched.monthIndex], 10) - 1;
  let parsedDay = parseInt(match[formatMatched.dayIndex], 10);

  let date = new Date();
  date.setFullYear(parsedYear);
  date.setMonth(parsedMonth);
...
function toDateString(value, format) {
  ...
  let validFormats = ['YYYY/MM/DD', 'DD/MM/YYYY', 'MM/DD/YYYY'];
...
function normalizeTime(value) {
  let regularexpTime = /^(0?[1-9]|1[0-2]):([0-5][0-9]):([0-5][0-9])\s?(AM|PM)?$/;
  let TimeRegularExp = /^(0?[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$/;
  let match = value.match(regularexpTime);
  let doesMatch = true;

  if (!match) {
    match = value.match(TimeRegularExp);
    doesMatch = false;
  }
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  ...
  if (extraData !== undefined && extraData.length > 0) {
    let presentExtraData = encodeURIComponent(extraData);
    endpoint += `/${presentExtraData}`;
  }
